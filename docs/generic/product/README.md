...
// HCons(head=1000, tail=HCons(head=900, tail=arrow.generic.HNil@7045c5a3))
```

```kotlin
Account(1000, 900).toHListLabeled()
// HCons(head=Tuple2(a=balance, b=1000), tail=HCons(head=Tuple2(a=available, b=900), tail=arrow.generic.HNil@7045c5a3))
```
...
// DeferredK(deferred=LazyDeferredCoroutine{New}@6b7a0b6f)
```
